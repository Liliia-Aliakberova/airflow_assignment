...
def extractip_to_txt():
    fh = open('/opt/airflow/the_logs/log.txt') 
    ...
    #write ip addresses
    fp = open(r'/opt/airflow/extracted_data.txt', 'w')
    for i in lst:
        fp.write("%s\n" % i)
...
def transformip_to_txt():
    ...
    fh = open('/opt/airflow/extracted_data.txt')
    fstring = fh.readlines()
    ...
    #write ip addresses
    fp = open(r'/opt/airflow/transformed_data.txt', 'w')
    for i in lst:
        fp.write("%s" % i)
...
#define the fourth task (load_data) 
load_data = BashOperator(
    task_id = 'load_data',
    bash_command = 'tar -cf /opt/airflow/weblog.tar  /opt/airflow/transformed_data.txt',
    dag = dag)
